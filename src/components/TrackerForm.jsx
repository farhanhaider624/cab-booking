...
export default function TrackerForm({
  ...
  const fetchBookings = async (email) => {
    try {
      ...
      const result = await axios.get("http://localhost:5500/api/bookings", {
        params: { email },
      });
  ...
  return (
    <>
      <div className="flex justify-center items-center min-h-[190px] shadow-lg max-w-[30%] bg-[#fde68a] rounded-lg p-4">
        <form onSubmit={handleSubmit} className="flex flex-1 flex-col">
          ...
          <button
            type="submit"
            className="bg-primary-btn font-medium hover:bg-primary-btn-hover text-white px-4 py-2 rounded-lg"
          >
            Track Booking
          </button>
        </form>
