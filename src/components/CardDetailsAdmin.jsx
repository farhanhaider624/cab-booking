...
export default function CardDetailsAdmin({
  cabId,
  name,
  price_per_min,
  time_to_arrive,
  ...
  const handleSave = (e) => {
    ...
    async function update() {
      const result = await axios.post("http://localhost:5500/api/cabs", {
        cabId,
        cabName,
        pricePerMin,
  ...
  return (
    <div className="bg-[#fde68a] rounded-lg p-4 flex flex-col w-[25%]  justify-between">
      <div className="flex justify-between flex-1">
        ...
        <p className="text-gray-700">₹{price_per_min}/min</p>
      </div>
      <div className="flex justify-between text-gray-800 items-end mt-8 flex-1">
        <p className="font-medium text-gray-800">
          Time to arrive: {time_to_arrive}
        </p>
        <button
          className="ml-4 bg-white text-[#9B7F11] hover:bg-white/70 rounded-lg px-4 py-2 font-bold"
          onClick={handleEdit}
        >
          Edit
        </button>
        ...
        <div className="absolute top-0 bottom-0 left-0 right-0 bg-black/40 flex justify-center items-center">
          <div className="flex bg-white rounded-lg drop-shadow-lg">
            <form className="p-4">
              ...
              <button
                type="submit"
                className="bg-red-500 ml-6 hover:bg-red-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
